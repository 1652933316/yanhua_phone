...
//   var intersect_text = start_content["intersect_text"].replaceAll('\n', '<br/>');
  console.log(intersect_text)
...
function init_is_typed() {
  ...
  if (
    ...
    if (img_src != null && typeof img_src != "undefined" && img_src != "") {
        if(img_src.indexOf('.mp4') != -1){
            ...
            video.src = img_src;
            video.play()
            $('#video_player').removeClass('hidden')
        } else {
      ...
      $("#div_is_img").hide();
      $("#div_is_inner").css("background", "rgba(255,255,255,.5)");
    }
  }
  try {
    //根据参数，设置背景色  by@wht
    var bgOpacity = GetIframeQueryString('img_opacity', 'myframe')
    if (!!bgOpacity) {
      $("#div_is_inner").css("background", "rgba(255,255,255," + bgOpacity + ")")
    }
  ...
  //但传参是类型5时候，隐藏缩略图 by@wht
  try {
    var nowPageType = GetIframeQueryString('type', 'myframe')
    console.log(nowPageType)
    if (!!nowPageType && nowPageType === 'no_thumb') {
      $("#div_is_img").hide()
  ...
  if (
    typeof start_content["img_bool"] == "undefined" ||
    typeof start_content["intersect_text"] == "undefined"
  ) {
    var random_img = random_img_as();
    if (
      random_img != null &&
      typeof random_img != "undefined" &&
      random_img != ""
    ) {
      $("#div_is_img").fadeIn("slow"); //
  ...
  var str_cursorChar;
  if (
    typeof start_content["cursor_char"] != "undefined" &&
    start_content["cursor_char"] != ""
  ) {
    switch (
    start_content["cursor_char"] //设置打字光标的样式
    ) {
      case "cursor_heart":
        str_cursorChar = "❤";
        break;
  ...
  if(start_content['txtType'] == 1){
      $('#span_is_typed').text(array_is_text.join('').replaceAll('<br/>',''))
      return
  }
...
function Init_is() {
  ...
  (cw_is = canvas_is.width = 500), (cx = cw_is / 2);
